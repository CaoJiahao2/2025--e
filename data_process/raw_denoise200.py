...
OUT_ROOT   = os.path.join(BASE_DIR, "data/200-envelope_denoised_12kHz")
REPORT_DIR = os.path.abspath("./200-reports")
FIG_DIR    = os.path.join(REPORT_DIR, "20-包络谱图")
...
def plot_envelope_figure(x0, x_bp, env, fs, f_env, P_env, flo, fhi, fr, bpfo, bpfi, bsf, out_png):
    Tshow = min(1.0, len(x0)/fs)  # 展示1秒
    ...
    idx = (f_env>=0.0) & (f_env<=200.0)  # 仅 0–200 Hz
    ax4.plot(f_env[idx], P_env[idx]); ax4.grid(True)
    ax4.set_title("包络谱（0–200 Hz）"); ax4.set_xlabel("f (Hz)"); ax4.set_ylabel("P_env")
    marks = [("fr",fr),("BPFO",bpfo),("BPFI",bpfi),("BSF",bsf)]
    for name, f0 in marks:
        if np.isfinite(f0) and 0.0 <= f0 <= 200.0:
            ax4.axvline(f0, ls="--"); ax4.text(f0, ax4.get_ylim()[1]*0.85, name, rotation=90, va="top")
